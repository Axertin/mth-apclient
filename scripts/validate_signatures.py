...
.text. With --expect-map, also checks Code entries resolve to the mapped address.
...
def main():
    ...
    for e in entries:
        ...
        if exp is not None and va != base + exp:
            print(f"ADDR   {name} expected {base + exp:#x} got {va:#x}"); failures += 1; continue
        print(f"OK     {name} -> {va:#x}")
        ok += 1

    print(f"\n{ok} ok, {failures} failure(s)")

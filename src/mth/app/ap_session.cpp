...
#endif
    coordinator_ = std::make_unique<ApCoordinator>(*link_, state, std::move(on_inbound_death), std::move(on_broadcast), std::move(on_scout),
                                                   std::move(on_session_reset), std::move(on_session_end));

...
void on_shop_set_cursor(void *shop_menu)
{
    ...
    if (name_w != nullptr)
    {
        ...
        // than being set up once. SetColor runs last: it raises the dirty flag that forces the
        // re-resolve through the palette we just swapped in.
        pal::shop_apply_name_palette(name_w, color);

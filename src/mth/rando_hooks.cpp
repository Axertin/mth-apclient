...
void repurpose_dummy_item()
{
    ...
    if (mprotect(reinterpret_cast<void *>(start), static_cast<std::size_t>(end - start), PROT_READ | PROT_WRITE) != 0)
    {
...
{
    // Require hook + Player* + position before accepting; return false to retry next tick.
...
void GameItemGranter::drain()
{
    ...
    // If readiness lapsed or position is non-finite, requeue for next drain.

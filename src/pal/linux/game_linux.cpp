...
bool install_newfile_kit_suppressor(NewfileKitSuppressFn should_suppress)
{
    ...
    // Clear first: it only maintains the re-entry guard, and installing it before the edge hook means the
    // guard is already live if a Clear lands between the two installs.
    ...
    if (g_kit_hook == kInvalidHookId)
        logf(LogLevel::Warn, "newfile-kit: SaveSlot::Clear not hooked; new-game edge may double-fire where Clear reaches InitGamestate");
